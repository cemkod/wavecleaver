...
func (b *FileBuilder) saveWithFilter() (string, int, error) {
	return chooseFileWithFilter("Save File", "Save", C.GTK_FILE_CHOOSER_ACTION_SAVE, b)
}
...
func chooseFileWithFilter(title string, buttonText string, action C.GtkFileChooserAction, b *FileBuilder) (string, int, error) {
	checkStatus()
	ctitle := C.CString(title)
	defer C.free(unsafe.Pointer(ctitle))
	cbuttonText := C.CString(buttonText)
	...
	filename := C.GoString(C.gtk_file_chooser_get_filename(fdlg))
	filterIdx := -1
	if active := C.gtk_file_chooser_get_filter(fdlg); active != nil {
		name := C.GoString(C.gtk_file_filter_get_name(active))
